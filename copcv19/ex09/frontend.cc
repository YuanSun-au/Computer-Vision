...
float  **g_u_bw;            /* displacement component in x-direction            */
float  **g_v_bw;            /* displacement component in y-direction            */
...
GLubyte *g_pixels;       /* pixel aray for Open-GL                           */
                         /*                                                  */
double **g_F;            /* estimated fundamental matrix                     */
int    g_norm_F;         /* use coordinate normalisation                     */
                         /****************************************************/
...
void showParams()
{
 ...
 print_console_header("Fundamental Matrix:");
 if (g_active_param==8)
     printf("\n (n) (Normalisation)             %s",(g_norm_F)?"ON":"OFF"); 
 else
     printf("\n (n)  Normalisation              %s",(g_norm_F)?"ON":"OFF"); 
...
{           
  ...
  HORN_SCHUNCK_MAIN(g_f1_s,g_f2_s,g_u,g_v,g_nx,g_ny,g_bx,g_by,
		    g_hx, g_hy, g_m_alpha, g_n_iter, g_n_omega,
...
//   HORN_SCHUNCK_MAIN(g_f2_s,g_f1_s,g_u_bw,g_v_bw,g_nx,g_ny,g_bx,g_by,
// 		    g_hx, g_hy, g_m_alpha, g_n_iter, g_n_omega,
// 		    g_n_warp_eta, g_n_warp_levels);
//   
//   float d_u,d_v;
//   
//   for (int i=g_bx;i<g_bx+g_nx;i++)
//   for (int j=g_by;j<g_by+g_ny;j++)
//     {
//     int ii = (int) floor(i + g_u[i][j]);
//     int jj = (int) floor(j + g_v[i][j]);
//   
//     if ((g_bx<=ii)&&(g_bx+g_nx>ii)&&(g_by<=jj)&&(g_by+g_ny>jj))
//       {
//       d_u = g_u[i][j] + g_u_bw[ii][jj];
//       d_v = g_v[i][j] + g_v_bw[ii][jj];
//       }
//     else {d_u = 10.f; d_v=10.f;}
//   
//     if ( ( d_u*d_u + d_v*d_v ) > 0.1f )
//       {
//       g_u[i][j] = 100.f;
//       g_v[i][j] = 100.f;
//       }
...
{   
  ...
  case GLUT_KEY_F1:
    ...
    printf(" F7     ........compute and write out fundamental matrix \n");
  ...
  case GLUT_KEY_F7:
    /* compute and write out fundamental matrix */
    compute_fundamental_matrix_TLS(g_u,g_v,g_nx,g_ny,g_bx,g_by,g_norm_F,g_F);   
    write_fundamental_matrix("matrix_e.fm",g_F);
    break;
...
ALLOC_MATRIX_DOUBLE(1, 3+2, 3+2, &g_F);
...
ALLOC_MATRIX(2, g_nx+2*g_bx, g_ny+2*g_by, &g_u_bw,    &g_v_bw);
...
glutCreateWindow("CORRESPONDENCE PROBLEMS - VISUALISATION FRONTEND - H");
...
FREE_MATRIX_DOUBLE (1, 3+2, 3+2, g_F);
...
FREE_MATRIX (2, g_nx+2*g_bx, g_ny+2*g_by, g_u_bw,    g_v_bw);
