...

{
    ...
    for (i = 0; i <= length; i++)
        conv[i] = 1 / (sigma / hx * sqrt(2.0 * 3.1415926)) * exp(-(i * i * hx * hx) / (2.0 * sigma * sigma));
    ...
    /* calculate entries of convolution vector */
    for (j = 0; j <= length; j++)
        conv[j] = 1 / (sigma / hy * sqrt(2.0 * 3.1415927)) * exp(-(j * j * hy * hy) / (2.0 * sigma * sigma));
...

{
    ...
    for (i = 1; i <= nx; i++)
        for (j = 1; j <= ny; j++)
        {
            // fx = 0.5*( u[j][i+1] - u[j][i-1] );
            // fy = 0.5*( u[j+1][i] - u[j-1][i] );
            fx = 0.5*( u[i+1][j] - u[i-1][j] );
            fy = 0.5*( u[i][j+1] - u[i][j-1] );
            u_mag[i][j] = pow(fx*fx + fy*fy, 0.5);
        }
