...

{
    ...
    for (i = 1; i <= nx; i++)
    {
        for (j = 1; j <= ny; j++)
        {
            ...
            df_dx = w1 * (f1[i + 1][j + 1] - f1[i - 1][j + 1] + f1[i + 1][j - 1] - f1[i - 1][j - 1]) + w2 * (f1[i + 1][j] - f1[i - 1][j]);

            w3 = 1.0 / (8.0 * hy);
            w4 = 1.0 / (4.0 * hy);

            df_dy = w3 * (f1[i + 1][j + 1] - f1[i + 1][j - 1] + f1[i - 1][j + 1] - f1[i - 1][j - 1]) + w4 * (f1[i][j + 1] - f1[i][j - 1]);
...

{
    ...
    for (i = 1; i <= nx; i++)
    {
        for (j = 1; j <= ny; j++)
        {
            ...
            if (trace <= eps){  // Rank ~ 0 -> No information
                u[i][j] = 0;
                v[i][j] = 0;
                c[i][j] = 0;
            
            }else if ( det <= eps ){    // Rank ~ 1 -> Only normal flow
                u[i][j] = - dxz[i][j] / ( dxx[i][j]*dxx[i][j] + dyy[i][j]*dyy[i][j] );
                v[i][j] = - dyz[i][j] / ( dxx[i][j]*dxx[i][j] + dyy[i][j]*dyy[i][j] );
                c[i][j] = 150;
            
            }else{  // Rank ~ 2 -> Full flow
                u[i][j] = ( -dxz[i][j]*dyy[i][j] + dyz[i][j]*dxy[i][j] ) / det;
                v[i][j] = ( -dyz[i][j]*dxx[i][j] + dxz[i][j]*dxy[i][j] ) / det;
                c[i][j] = 255;
            }
            
            // c[i][j] = det - 0.1*trace*trace;
        }
    }
