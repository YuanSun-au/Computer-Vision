	...
	for (i = bx; i < nx + bx; i++)
		for (j = by; j < ny + by; j++)
		{
			...
			if (coeff > 0.0) //dilation
				idm_update[i][j] =
					sqrt((sq(min(v[i][j] - v[i - 1][j], 0)) + sq(max(v[i + 1][j] - v[i][j], 0))) * hx_1 + (sq(min(v[i][j] - v[i][j - 1], 0)) + sq(max(v[i][j + 1] - v[i][j], 0))) * hy_1) * fabs(coeff);
			else //erosion
				idm_update[i][j] =
					-sqrt((sq(max(v[i][j] - v[i - 1][j], 0)) + sq(min(v[i + 1][j] - v[i][j], 0))) * hx_1 + (sq(max(v[i][j] - v[i][j - 1], 0)) + sq(min(v[i][j + 1] - v[i][j], 0))) * hy_1) * fabs(coeff);
		}
	return;
}
	...
	for (i = bx; i < nx + bx; i++)
		for (j = by; j < ny + by; j++)
		{
			...
			else
			{
				...
				mcm_update[i][j] = ( vy*vy*vxx - 2*vx*vy*vxy + vx*vx*vyy ) / nabla_v_sq;
				/*!      !*/
			}
		}
	return;
	...
	float tau)
{
	...
	for (i = bx; i < nx + bx; i++)
		for (j = by; j < ny + by; j++)
		{
			...
			v_new[i][j] = v[i][j] + tau * ( mcm_update[i][j] + 1/lambda*idm_update[i][j] );
			/*!      !*/
		}
	disalloc_matrix(mcm_update, nx + 2 * bx, ny + 2 * by);
	...
	if (!strcmp(argv[1], "cv"))
	{
		...
		while (T > tau)
		{
			...
			if (c == delta)
			{
				...
				n++;
				if (n < 10)
					sprintf(interm, "0000%d_%s", n, out);
				else if (n < 100)
					sprintf(interm, "000%d_%s", n, out);
				else if (n < 1000)
					sprintf(interm, "00%d_%s", n, out);
				else if (n < 10000)
					sprintf(interm, "0%d_%s", n, out);
				else
					sprintf(interm, "%d_%s", n, out);
				ptr = fopen(interm, "wb");
				fprintf(ptr, "P5 \n");
				fprintf(ptr, "%ld %ld \n255\n", nx, ny);
				fclose(ptr);
		...
		for (i = bx; i < nx + bx; i++)
			for (j = bx; j < nx + bx; j++)
			{
				if (v_out[i][j] == 255.0)
				{
					if ((v_out[i - 1][j] == 0) || (v_out[i + 1][j] == 0) || (v_out[i][j - 1] == 0) || (v_out[i][j + 1] == 0) || (v_out[i - 1][j - 1] == 0) || (v_out[i - 1][j + 1] == 0) || (v_out[i + 1][j - 1] == 0) || (v_out[i + 1][j + 1] == 0))
					{
						fs[i][j] = 255.0;
						f[i][j] = 0.0;
					}
